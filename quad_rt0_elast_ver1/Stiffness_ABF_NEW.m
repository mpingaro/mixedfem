function [AELEM,BELEM,CELEM,b_load] = Stiffness_ABF_NEW(point,f,s,cf)  
...
for k = 1:npg
    ...
    sig(:,6) = JJ'*[(3*x^2-1)*(1-y^2); -2*y*(x^3-x) ] ; % new bouble function component 1
    sig(:,7) = JJ'*[-2*x*(y-y^3); (1-3*y^2)*(1-x^2) ] ; % new bouble function conponent 2   
